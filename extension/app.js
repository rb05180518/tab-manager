...
document.addEventListener('click', (e) => {
  if (e.target.id === 'confirmOverlay') { closeConfirm(false); return; }
  if (e.target.id === 'confirmOk')     { closeConfirm(true);  return; }
  if (e.target.id === 'confirmCancel') { closeConfirm(false); return; }
});
...
function smartTitle(title, url) {
  ...
  if (hostname === 'github.com' || hostname === 'www.github.com') {
    ...
    if (parts.length >= 2) {
      ...
      if (rest[0] === 'issues' && rest[1]) return `${owner}/${repo} 议题 #${rest[1]}`;
...
function renderDomainCard(group) {
  ...
  const pageChips = visibleTabs.map(tab => {
    ...
    const groupTag = groupId ? ` <span class="chip-grouped-badge">已分组</span>` : '';
    return `<div class="page-chip clickable${chipClass}" draggable="true" data-action="focus-tab" data-tab-url="${safeUrl}" title="${safeTitle}">
      <span class="chip-select" data-action="toggle-select" data-tab-url="${safeUrl}" title="选择"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5" /></svg></span>
      ${faviconUrl ? `<img class="chip-favicon" src="${faviconUrl}" alt="" onerror="this.style.display='none'">` : ''}
...
function renderSearchResults(query) {
  ...
  if (matches.length === 0) {
    resultsEl.innerHTML = '<div class="search-empty">没有找到匹配的标签页</div>';
  } else {
...
function renderCustomGroups() {
  ...
  container.innerHTML = customTabGroups.map(group => {
...
async function removeUrlFromAllGroups(url) {
  ...
  let changed = false;
  for (const g of groups) {
    if (g.tabUrls.includes(url)) { g.tabUrls = g.tabUrls.filter(u => u !== url); changed = true; }
  }
...
function toggleSelection(url, forceState) {
  if (!url) return;
  const selected = forceState !== undefined
    ? forceState
    : !_selectedTabUrls.has(url);
  if (selected) _selectedTabUrls.add(url);
  else _selectedTabUrls.delete(url);

  // Update all chips with this URL
  document.querySelectorAll(`.page-chip[data-tab-url="${url.replace(/"/g, '\\"')}"]`).forEach(chip => {
...
document.addEventListener('dragstart', (e) => {
  const chip = e.target.closest && e.target.closest('.page-chip[draggable="true"]');
  if (!chip) return;
  ...
  if (e.dataTransfer) {
    ...
    try { e.dataTransfer.setData('text/plain', _draggedUrls.join('\n')); } catch {}
  }
});
...
document.addEventListener('dragend', (e) => {
  ...
  document.querySelectorAll('.custom-group-card.drag-over').forEach(c => c.classList.remove('drag-over'));
});
...
document.addEventListener('drop', async (e) => {
  ...
  clearSelection();
  renderCustomGroups();
  showToast(urls.length > 1 ? `已把 ${urls.length} 个标签页加入分组` : '已加入分组');
});

// ---- Multi-select click handlers ----
document.addEventListener('click', (e) => {
  // Toggle a chip's selection (the select box)
  const selectBox = e.target.closest && e.target.closest('[data-action="toggle-select"]');
  if (selectBox) {
    e.stopPropagation();
    e.preventDefault();
...
document.addEventListener('click', async (e) => {
  ...
  // Delete a group
  const deleteBtn = e.target.closest && e.target.closest('[data-action="delete-group"]');
  if (deleteBtn) {
    const groupId = deleteBtn.dataset.groupId;
    const group = customTabGroups.find(g => g.id === groupId);
    const name = group ? group.name : '该分组';
    if (!await confirmAction(`删除分组「${name}」?(不会关闭里面的标签页)`)) return;
  ...
  const removeBtn = e.target.closest && e.target.closest('[data-action="remove-from-group"]');
  if (removeBtn) {
  ...
  const closeGroupBtn = e.target.closest && e.target.closest('[data-action="close-group-tabs"]');
  if (closeGroupBtn) {
    ...
    if (!await confirmAction(`${verb}分组「${name}」下的全部 ${urls.length} 个标签页${tail}`)) return;
...
}, true); // capture so blur fires on contenteditable
...
document.addEventListener('click', async (e) => {
  ...
  if (action === 'close-single-tab') {
    ...
    if (!await confirmAction(syncClose ? '关闭这个标签页?' : '从面板移除这个标签页?(浏览器标签页保留)')) return;
  ...
  if (action === 'defer-single-tab') {
    ...
    if (!await confirmAction(syncClose ? '保存并关闭这个标签页?' : '保存并从面板移除这个标签页?(浏览器标签页保留)')) return;
  ...
  if (action === 'close-domain-tabs') {
    ...
    const verb = syncClose ? '关闭' : '从面板移除';
    const tail = syncClose ? '' : '?(浏览器标签页保留)';
    if (!await confirmAction(`${verb}「${groupLabel}」下的全部 ${urls.length} 个标签页${tail || '?'}`)) return;
  ...
  if (action === 'dedup-keep-one') {
    ...
    if (!await confirmAction(`关闭 ${urls.length} 个重复标签页(每个保留一份)?`)) return;
  ...
  if (action === 'close-all-open-tabs') {
    ...
    const tail = syncClose ? '?' : '?(浏览器标签页保留)';
    if (!await confirmAction(`${verb} ${allUrls.length} 个打开的标签页${tail}`)) return;
...
function isDashboardBusy() {
  ...
  if (document.activeElement) {
    ...
    if (el.classList && (el.classList.contains('group-name') ||
        el.classList.contains('new-group-input'))) return true;
  }
...
document.addEventListener('change', (e) => {
  if (e.target.id === 'syncCloseToggle') {
    ...
    showToast(e.target.checked ? '已开启:关闭时同步关闭浏览器标签页' : '已关闭:仅从面板移除,浏览器标签页保留');
  }
});
