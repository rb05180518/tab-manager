...
1. **不再接管新标签页** — 新开 tab 恢复 Chrome 默认;点击工具栏图标才打开 Tab Manager
...
新增/改写:监听 `chrome.action.onClicked`,打开或聚焦 Tab Manager 页面:
...
- 这样避免重复打开多个 Tab Manager 页(顺带也降低原来 dupe banner 的作用,但 banner 逻辑保留兼容)

### 文档更新

- `README.md` / `AGENTS.md`:把"打开一个新标签页就能看到"改为"点击工具栏的 Tab Manager 图标打开"
...
| 关闭多余 Tab Manager `close-tabout-dupes`  | "关闭多余的 Tab Manager 标签页?"          |
